...
$(document).on('click', '.mail', function(e){
    ...
    KR.Phone.Functions.SetupMail(MailData);
...
KR.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-mail").html(KR.Phone.Data.PlayerData.firstname+"."+KR.Phone.Data.PlayerData.name+"@Flash.ro");
    $("#mail-header-lastsync").html("Last synchronized "+MessageTime);
    if (Mails !== null && Mails !== undefined) {
        if (Mails.length > 0) {
            $(".mail-list").html("");
            $.each(Mails, function(i, mail){
                var element = '<div class="mail" id="mail-'+mail.mailid+'"><span class="mail-sender" style="font-weight: bold;">'+mail.sender+'</span> <div class="mail-text"><p>'+mail.message+'</p></div></div>';
...
KR.Phone.Functions.SetupMail = function(MailData) {
...
$(document).on('click', '#new-advert-submit', function(e){
    ...
    if (Advert !== "") {
        ...
        KR.Phone.Notifications.Add("fas fa-ad", "Advertisement", "You can\'t post an empty ad!", "#ff8f1a", 2000);
    }
});

KR.Phone.Functions.RefreshAdverts = function(Adverts) {
    $("#advert-header-name").html("@"+KR.Phone.Data.PlayerData.firstName+""+KR.Phone.Data.PlayerData.secondName+" | "+KR.Phone.Data.PlayerData.phone);
    if (Adverts.length > 0 || Adverts.length == undefined) {
        $(".advert-list").html("");
        $.each(Adverts, function(i, advert){

            if (advert.url) {
                var element = `<div class="advert"><span class="advert-sender">${advert.name} | ${advert.number}</span><p>${advert.message}</p></br><img class="advimage" src=`+advert.url +` style=" border-radius:4px; width: 95%; position:relative; z-index: 1; right:1px;height: auto; bottom:1vh;"><span><div class="adv-icon"></div> </span></div>`;
            } else {
        ...
        $(".advert-list").html("");
        var element = '<div class="advert"><span class="advert-sender">Nu sunt anunturi</span></div>';
        $(".advert-list").append(element);
    }
}
$(document).on('click','#adv-delete',function(e){
    ...
    $.post('https://phone/DeleteAdvert', function(){
        setTimeout(function(){
            KR.Phone.Notifications.Add("fas fa-ad", "Advertisement", "The ad was deleted", "#ff8f1a", 2000);
        },400)
    });
})
