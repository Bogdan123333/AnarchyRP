...
KR.Phone.Functions.SetupRecentCalls = function(recentcalls) {
...
KR.Phone.Functions.writeNumberInDial = function(number) {
	if(KR.Phone.Data.currentApplication !== "phone") {
		if(KR.Phone.Data.currentApplication) {
			...
			setTimeout(function(){
				...
				setTimeout(function() {
					...
					KR.Phone.Functions.ToggleApp("phone", "block");
				}, 200)
	...
	KR.Phone.Data.currentApplication = "phone";
            ...
            if (status.IsOnline) {
                if (status.CanCall) {
                    if (!status.InCall) {
                        ...
                        KR.Phone.Notifications.Add("fas fa-phone", "Phone", "Esti deja intr-un apel!");
                    }
                } else {
                    KR.Phone.Notifications.Add("fas fa-phone", "Phone", "Acesta persoana este ocupata!");
                }
            } else {
                KR.Phone.Notifications.Add("fas fa-phone", "Phone", "Aceasta persoana nu este disponibila!");
            }
        } else {
        ...
        if (cData.number !== KR.Phone.Data.PlayerData.phone) {
            if (status.IsOnline) {
                if (status.CanCall) {
                    ...
                    KR.Phone.Notifications.Add("fas fa-phone", "Phone", "Aceasta persoana este ocupata!");
                }
            } else {
                KR.Phone.Notifications.Add("fas fa-phone", "Phone", "Aceasta persona nu este disponibila!");
            }
        } else {
            KR.Phone.Notifications.Add("fas fa-phone", "Phone", "Nu iti poti apela propriul numar!");
        }
...
KR.Phone.Functions.LoadContacts = function(myContacts) {
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== KR.Phone.Data.PlayerData.phone) {
        ...
        setTimeout(function(){
            ...
            KR.Phone.Functions.ToggleApp("phone", "none");
            KR.Phone.Functions.ToggleApp("whatsapp", "block");
            KR.Phone.Data.currentApplication = "whatsapp";

            $.post('https://phone/GetWhatsappChat', JSON.stringify({phone: ContactData.number}), function(chat){
                KR.Phone.Functions.SetupChatMessages(chat, {
        ...
        KR.Phone.Notifications.Add("fa fa-phone-alt", "Phone", "Nu iti poti da whatsapp tie..", "default", 3500);
    }
});
...
$(document).on('click', '#edit-contact', function(e){
    ...
    KR.Phone.Animations.TopSlideDown(".phone-edit-contact", 200, 0);
});
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('https://phone/EditContact', JSON.stringify({
            ...
            KR.Phone.Functions.LoadContacts(PhoneContacts);
        });
        KR.Phone.Animations.TopSlideUp(".phone-edit-contact", 250, -100);
        setTimeout(function(){
            $(".phone-edit-contact-number").val("");
            $(".phone-edit-contact-name").val("");
        }, 250)
    } else {
        KR.Phone.Notifications.Add("fas fa-exclamation-circle", "Editeaza Contact", "Completati toate campurile!");
    }
});
...
$(document).on('click', '#phone-share-icon', function(){
    $.post('https://phone/shareContact', JSON.stringify({}));
});
$(document).on('click', '#phone-plus-icon', function(e){
    e.preventDefault();

    KR.Phone.Animations.TopSlideDown(".phone-add-contact", 200, 0);
});
...
$(document).on('click', '#add-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        ...
        KR.Phone.Animations.TopSlideUp(".phone-add-contact", 250, -100);
        setTimeout(function(){
        ...
        KR.Phone.Notifications.Add("fas fa-exclamation-circle", "Adauga contact", "Completeaza toate campurile!");
    }
});
        ...
        if (cData.number !== KR.Phone.Data.PlayerData.phone) {
            if (status.IsOnline) {
                if (status.CanCall) {
                    ...
                    KR.Phone.Notifications.Add("fas fa-phone", "Phone", "Aceasta persoana este deja intr-un apel");
                }
            } else {
                KR.Phone.Notifications.Add("fas fa-phone", "Phone", "Aceasta persoana nu este diponibila!");
            }
        } else {
            KR.Phone.Notifications.Add("fas fa-phone", "Phone", "Nu iti apela propriul numar!");
        }
    });
}

CancelOutgoingCall = function() {
    if (KR.Phone.Data.currentApplication == "phone-call") {
        KR.Phone.Animations.TopSlideUp('.phone-application-container', 400, -160);
        KR.Phone.Animations.TopSlideUp('.'+KR.Phone.Data.currentApplication+"-app", 400, -160);
        setTimeout(function(){
            KR.Phone.Functions.ToggleApp(KR.Phone.Data.currentApplication, "none");
...
IncomingCallAlert = function(CallData, Canceled, AnonymousCall) {
    if (!Canceled) {
        if (!KR.Phone.Data.CallActive) {
            KR.Phone.Animations.TopSlideUp('.phone-application-container', 400, -160);
            KR.Phone.Animations.TopSlideUp('.'+KR.Phone.Data.currentApplication+"-app", 400, -160);
            setTimeout(function(){
                var Label = "Aveti un apel de la: "+CallData.name
                if (AnonymousCall) {
                    Label = "Esti sunat de o persoana anonima"
                }
                $(".call-notifications-title").html("Apel");
                ...
                $(".phone-app").css({"display":"none"});
                KR.Phone.Functions.HeaderTextColor("white", 400);
                $("."+KR.Phone.Data.currentApplication+"-app").css({"display":"none"});
...
KR.Phone.Functions.SetupCurrentCall = function(cData) {
    if (cData.InCall) {
        CallData = cData;
...
$(document).on('click', '.phone-currentcall-container', function(e){
    ...
    KR.Phone.Functions.HeaderTextColor("white", 500);
    KR.Phone.Animations.TopSlideDown('.phone-application-container', 500, 0);
    KR.Phone.Animations.TopSlideDown('.phone-call-app', 500, 0);
    KR.Phone.Functions.ToggleApp("phone-call", "block");
...
KR.Phone.Functions.AnswerCall = function(CallData) {
    $(".phone-call-incoming").css({"display":"none"});
    $(".phone-call-outgoing").css({"display":"none"});
    $(".phone-call-ongoing").css({"display":"block"});
...
KR.Phone.Functions.SetupSuggestedContacts = function(Suggested) {
