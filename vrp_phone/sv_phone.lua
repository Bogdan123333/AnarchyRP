...
local webhook = "https://discord.com/api/webhooks/1158670482141360180/h0NDpvVB6DuOpJMKzRcESQ0xC39JOSyOkIlDiE2G27bLKfZfbAC3fXSd2VXVEVgL2DNk"
...
function vRPphone.hasPhone()
    ...
    if (vRP.getInventoryItemAmount({user_id, "phone"}) or 0) > 0 then
        return true
    end
    ...
    if holder then
        ...
        if holder_src then
            ...
            if vRP.tryBankPayment({user_id, amount, false, "Bank Transfer (iban: "..iban..")"}) then
                ...
                TriggerClientEvent("vrp-phone:notify", holder_src, "$ has been transferred to you"..vRP.formatMoney({amount}))
                
                return "You transferred $"..vRP.formatMoney({amount})
...
RegisterServerEvent("vrp-phone:shareImage", function(shareTbl)
    ...
    TriggerClientEvent("vrp-phone:notify", player, "Image has been shared!")
...
function vRPphone.suggestContact()
    ...
    vRPclient.getNearestPlayer(player, {5}, function(nplayer)
        if nplayer then
            ...
            TriggerClientEvent("vrp-phone:notify", nplayer, "You have a new contact suggestion!")
            TriggerClientEvent("vrp-phone:notify", player, "You have suggested a new contact!")
        
        else
            TriggerClientEvent("vrp-phone:notify", player, "No person around you!")
        end
    end)
