    ...
    if phoneData.number and not phoneOn then
        vRPphone.hasPhone({}, function(hasPhone)
            if not hasPhone then
                vRP.notify({"You don't have a mobile phone", "error"})
            else
...
RegisterKeyMapping('phone', 'Open mobile phone', 'keyboard', 'L')
