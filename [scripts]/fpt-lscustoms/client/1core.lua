...
vRPserver = Tunnel.getInterface("vRP","esk_tuning")
vRPtuning = Tunnel.getInterface("esk_tuning","esk_tuning")
vRPtuningC = {}
Tunnel.bindInterface("esk_tuning",vRPtuningC)
vRPStuning = Tunnel.getInterface("esk_tuning","esk_tuning")
...
local myMoney = 0
vRPtuningC.updateMyMoney = function(money)
    myMoney = money
end
...
local notified, inRange = false, false
...
CreateThread(function()
    for k,v in pairs (Config.Positions) do 
        ...
        local tempBlip = AddBlipForCoord(v[1], v[2], v[3])
        
        SetBlipSprite(tempBlip, 72)
        SetBlipDisplay(tempBlip, 4)
        SetBlipScale(tempBlip, 0.6)
        ...
        if (not uiOpen) then
            if ((playerVeh ~= 0) and (DoesEntityExist(playerVeh)) and (GetPedInVehicleSeat(playerVeh, -1) == playerPed)) then
                ...
                for i = 1, #Config.Positions, 1 do
                    ...
                    local tempPos = Config.Positions[i]
                        ...
                        local tempDist = #(playerPos - vector3(tempPos[1], tempPos[2], tempPos[3]))
                        ...
                        if (tempDist <= tempActionDist) then
                            ...
                            if (not IsHudHidden()) then
                                ...
                                if not notified then
                                    ...
                                    exports['fpt-scripts']:Open('[E] - Modifica vehiculul', 'darkblue', 'left')
                                end
                            end
            ...
            if (customConfigPosIndex) then
                ...
                if (playerVeh == 0 or playerVeh ~= customVehicle) then
                    ...
                    local tempDist = GetDistanceBetweenCoords(playerPos.x, playerPos.y, playerPos.z, tempPos[1], tempPos[2],tempPos[3], true)
                    if (tempDist > tempActionDist) then
                        closeUI(1, 1)
                    end
                end
...
function openUI()
    ...
    if (playerVeh ~= 0 and DoesEntityExist(playerVeh)) then
        ...
        customCamMain = CreateCamWithParams('DEFAULT_SCRIPTED_CAMERA', camPos.x, camPos.y, camPos.z - 1, -15.0, 0.0, headingToObject, GetGameplayCamFov(), false, 2)
        customCamSec = CreateCamWithParams('DEFAULT_SCRIPTED_CAMERA', camPos.x, camPos.y, camPos.z - 1, -15.0, 0.0, headingToObject, GetGameplayCamFov(), false, 2)
            ...
            elseif (data.user == 'enter') then
                ...
                if ((not isOpenByAdmin) and menuOption and menuOption.price) then
                    ...
                    elseif (menuOption.price > 0) then
                        ...
                        if (customConfigPosIndex and customConfigPosIndex > 0 and customConfigPosIndex <= #Config.Positions and Config.Positions[customConfigPosIndex] and Config.Positions[customConfigPosIndex]) then
                            tempPrice = tempPrice * 2
                        end
                        if (myMoney >= tempPrice) then
                            canBuyMod = true
