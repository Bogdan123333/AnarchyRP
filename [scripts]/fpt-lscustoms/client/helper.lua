...
function saveTuning()
	...
	vRPStuning.saveTuning({tempData})
	--return tempData
end

function vRPtuningC.setTuningOnVehicle(tuning)
    veh = GetVehiclePedIsUsing(PlayerPedId())
	SetVehicleData(veh,tuning)
end
...
function repairVehicle(vehicle)
	-- if not isOpenByAdmin then
		...
		-- exports['fpt-ui']:progressBar("Un mecanic iti repara masina", fixingTime)
		exports['fpt-ui']:Progress({
			duration = fixingTime,
			label = "Un ucenic iti repara masina",
			useWhileDead = false,
			canCancel = true,
			controlDisables = {
        ...
        label = "Un ucenic iti spala masina",
        useWhileDead = false,
        canCancel = true,
        controlDisables = {
            disableMovement = false,
