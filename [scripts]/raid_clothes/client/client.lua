...
Citizen.CreateThread(function()
	...
	addBlipsForCateg(cfg.tattoosShops, {75, 37, "Tattoo Shop"})
end)
...
function SetSkin(model, setDefault)
	...
	if IsModelInCdimage(model) and IsModelValid(model) then
		...
		local weapons = exports.plesWeapons:getWeapons()
		local health = vRP.getHealth({})
		SetPlayerModel(PlayerId(), model)
		if weapons then
			exports.plesWeapons:setWeapons(weapons)
		end
...
function Save(save)
	if save then
		...
		--if data.model == 'mp_f_freemode_01' or data.model == 'mp_m_freemode_01' or data.model == '1885233650' then
		...
		if menu ~= nil then
			...
			if not enabled then
				...
				if not inputActive then
					inputActive = true
					TriggerEvent("vrp-hud:showBind", {key = "E", text = menu[2]})
				end
