...
$(document).on('click', '.twitter-header-tab', function (e) {
    ...
    if (PressedTwitterTab !== CurrentTwitterTab) {
        ...
        if (PressedTwitterTab == "twitter-self") {
            $.post('http://phone/GetSelfTweets', JSON.stringify({}), function (selfdata) {
                KR.Phone.Notifications.LoadSelfTweets(selfdata)
            })
        ...
        $.post('http://phone/GetSelfTweets', JSON.stringify({}), function (selfTweets) {
            KR.Phone.Notifications.LoadSelfTweets(selfTweets)
        })
    }
...
$(document).on('click', '.twitter-new-tweet', function (e) {
    e.preventDefault();
    KR.Phone.Animations.TopSlideDown(".twitter-new-tweet-tab", 450, 0);
});

KR.Phone.Notifications.LoadTweets = function (Tweets) {
...
KR.Phone.Notifications.LoadSelfTweets = function (Tweets) {
    Tweets = Tweets.reverse();
...
$(document).on('click', '.tweet-delete', function (e) {
    ...
    //console.log(id)
    $.post('http://phone/DeleteTweet', JSON.stringify({
        id: id,
    }));

    $.post('http://phone/GetSelfTweets', JSON.stringify({}), function (tweets) {
        KR.Phone.Notifications.LoadSelfTweets(tweets);
    });
    $.post('http://phone/GetTweets', JSON.stringify({}), function (tweets) {
        KR.Phone.Notifications.LoadTweets(tweets);
    });
...
KR.Phone.Notifications.LoadMentionedTweets = function (Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets.length > 0) {
...
KR.Phone.Functions.FormatTwitterMessage = function (TwitterMessage) {
...
$(document).on('click', '#send-tweet', function (e) {
    ...
    if (TweetMessage != "" || TweetUrl != "") {
        ...
        KR.Phone.Animations.TopSlideUp(".twitter-new-tweet-tab", 450, -120);
        $('#tweet-new-url').val("")
        $("#tweet-new-message").val("");
    } else {
        KR.Phone.Notifications.Add("fab fa-twitter", KR.Phone.Functions.Lang("TWITTER_TITLE"), KR.Phone.Functions.Lang("TWITTER_ENTER_MSG"), "#1DA1F2");
    }
});
...
$(document).on('click', '.hashtag-tag-text', function (e) {
    ...
    if (!HashtagOpen) {
        ...
        $.post('http://phone/GetHashtagMessages', JSON.stringify({
            hashtag: Hashtag
...
function CopyMentionTag(elem) {
    ...
    KR.Phone.Notifications.Add("fab fa-twitter", KR.Phone.Functions.Lang("TWITTER_TITLE"), $(elem).data('mentiontag') + " Kopyalandı!", "rgb(27, 149, 224)", 1250);
...
KR.Phone.Notifications.LoadHashtagMessages = function (Tweets) {
    ...
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
        ...
        $.each(Tweets, function (i, Tweet) {
            var TwtMessage = KR.Phone.Functions.FormatTwitterMessage(Tweet.message);
            var today = new Date();
            var TweetTime = new Date(Tweet.time);
            var diffMs = (today - TweetTime);
            var diffDays = Math.floor(diffMs / 86400000);
