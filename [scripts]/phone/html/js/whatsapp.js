...
$(document).on('click', '.whatsapp-chat', function(e){
    ...
    KR.Phone.Functions.SetupChatMessages(ChatData);
...
KR.Phone.Functions.GetLastMessage = function(messages) {
...
KR.Phone.Functions.LoadWhatsappChats = function(chats) {
    $(".whatsapp-chats").html("");
    $.each(chats, function(i, chat){
        ...
        var LastMessage = KR.Phone.Functions.GetLastMessage(chat.messages);
...
KR.Phone.Functions.ReloadWhatsappAlerts = function(chats) {
    $.each(chats, function(i, chat){
        if (chat.Unread > 0 && chat.Unread !== undefined && chat.Unread !== null) {
            $(".unread-chat-id-"+i).html(chat.Unread);
            $(".unread-chat-id-"+i).css({"display":"block"});
...
$(document).on('click', '#whatsapp-openedchat-send', function(e){
    ...
    if (Message !== null && Message !== undefined && Message !== "") {
        ...
        KR.Phone.Notifications.Add("fab fa-whatsapp", "Whatsapp", "Nu poti trimite un mesaj gol!", "#25D366", 1750);
    }
});
...
$(document).on('click', '#delete-wapp', function(e){
    e.preventDefault();

    $.post('http://phone/DeleteWapp', JSON.stringify({
        ChatNumber: OpenedChatData.number,
    }));
    OpenedChatData.number = null;
    $(".whatsapp-chats").css({"display":"block"});
...
// KR.Phone.Functions.SetupChatMessages = function(cData, NewChatData) {
//     if (cData) {
...
//                 var Sender = "me";
//                 if (message.sender !== KR.Phone.Data.PlayerData.id) { Sender = "other"; }
//                 var MessageElement
...
KR.Phone.Functions.SetupChatMessages = function(cData, NewChatData) {
    if (cData) {
        ...
        $.each(cData.messages, function(i, chat){
            ...
            $.each(cData.messages[i].messages, function(index, message){
                ...
                if (message.type == "message") {
                    ...
                    MessageElement = '<div class="whatsapp-openedchat-message whatsapp-openedchat-message-'+Sender+'" style = "max-width: 100%;"> <img src ="'+message.message+'" style = "height:11.4vh; border-radius:7px;"></img><div class="whatsapp-openedchat-message-time">'+message.time+'</div></div><div class="clearfix"></div>'
                }
        ...
        var NewDateYear = NewDate.getFullYear();
        var DateString = ""+NewDateDOM+"-"+(NewDateMonth+1)+"-"+NewDateYear;
        var ChatDiv = '<div class="whatsapp-openedchat-messages-'+DateString+' unique-chat"><div class="whatsapp-openedchat-date">ASTAZI</div></div>';
