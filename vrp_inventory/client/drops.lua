...
RegisterNetEvent("fp-inventory:refreshDrops")
AddEventHandler("fp-inventory:refreshDrops", function(x)
    droppedItems = x
end)
    ...
    while true do
        ...
        for _, dropData in pairs(droppedItems) do
            ...
            if dist <= 8 then
                ...
                if dist <= 2 then
                    DrawText3D(pos.x, pos.y, pos.z + 0.01, "~b~E ~w~- "..dropData.label.." ~w~[~b~"..dropData.amount.."~w~]", 0.55)

                    if IsControlJustReleased(0, 51) then
                        TriggerServerEvent("fp-inventory:collectDrop", _)
                    end
                end
